...
const FeaturedPost = ({user, token, posts, setPosts, featuredPost, setFeaturedPost}) => {
    const [featuredPost, setFeaturedPost] = useState('')
    useEffect(() => {
        const apiCall = async () => {
          const result = await fetch('https://strangers-things.herokuapp.com/api/2204-FTB-MT-WEB-PT/posts/${postId});
